...
protected:
  ...
  /// 使い方の説明です。
	...
	static StringHash<lilfes_flag> flagCollection;
...
public:
	lilfes_flag(const char *iname, const char *iusage) : opt('\0'), name(iname), usage(iusage) { flagCollection.Add(this); }
  /// <var>iname</var>：長いオプション名<br>
  /// <var>iusage</var>：使い方の説明<br>

	lilfes_flag(uchar iopt, const char *iname, const char *iusage) : opt(iopt), name(iname), usage(iusage) { flagmap[opt] = this; flagCollection.Add(this); }
  /// <var>iopt</var>：短いオプション名<br>
  /// <var>iname</var>：長いオプション名<br>
  /// <var>iusage</var>：使い方の説明<br>
        virtual ~lilfes_flag() { }

	const char *GetKey() { return name; }
  ...
  /// <ref>lilfes_int_flag</ref>など整数値の引数をとることができるオプションに、整数値を設定する仮想関数です。
	virtual const char *GetStrVal() { return 0; }
  /// 文字列の引数をとることができるオプションから、文字列を取得する仮想関数です。
	virtual void SetStrVal(const char *) { }
  ...
  /// そのオプションに文字列<var>s</var>をオプションの引数として渡します。引数として使われればtrueが、使われなければfalseが返ります。
	virtual FSP GetFlagToFSP(machine &m) { return FSP(m); }
  ...
  /// そのオプションの使い方の説明を表示します。
	...
	static void ParseOptions(machine *mach, int argc, char **argv);
  /// 最初に渡されたオプション文字列を解析し、適切なlilfes_flagオブジェクトに値を渡すメソッド。

	static int ExecOptions(machine &mach);
  /// -eオプションで指定されたコマンドを解析し、実行するメソッド。

	static lilfes_flag * Search(const char *s) { return flagCollection.Find(s);}
...
public:
  ...
  /// <var>iusage</var>：使い方の説明<br>
	...
	virtual int GetIntVal() { return val; }
  /// オプションの引数（整数型）を取得します。
  ...
  /// そのオプションに文字列sをオプションの引数として渡します。 引数として使われればtrueが、使われなければfalseが返ります。
	virtual FSP  GetFlagToFSP(machine &m) { return FSP(m, (mint)val); }
  /// ???
};
/// </body></classdef>
...
#ifdef PROFILE
    profiler::SetQuiet(true);
#endif // PROFILE
    return true;
  }
  virtual FSP GetFlagToFSP(machine &m);
};
/// </body></classdef>
...
public:
  ...
  /// qsortを利用してオプション名をソートするためのオプション名の比較関数。
...
public:
  ...
  static bool SetFlagFromStringArray(int argc, char **argv);
