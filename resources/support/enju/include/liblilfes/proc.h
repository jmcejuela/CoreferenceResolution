...
class procedure
{
  ...
  /// LiLFeSの中のデバッグ機能の中でその述語をtraceするかどうかを表します。
	...
	static VarArray<procpair> serialtable;
        static IntegerHash<procedure> hash; /// key = procedureのreptypeのtserial  value = procedure
...
public:
  ...
  /// ハッシュ用のメソッドです。キーとしてその述語の表現タイプのシリアル番号を取得します。
	const char *GetName() const { return reptype->GetName(); }
  /// その述語の表現タイプの名前を取得します。
  ...
  /// 表現タイプが<var>n</var>、引数の数が<var>arity</var>、procthunkが<var>thunk</var>の述語を作ります。
	static procedure* Search(const type *t) 
			{ procedure *r = hash.Find(t->GetSerialNo()); return r; }
 ...
 /// シリアル番号から対応するprocthunkオブジェクトを取得します。
  ...
  /// procthunk<var>pt</var>に対応するprocedureオブジェクトを取得します。
	static uint GetSerialCount() { return serialtable.GetSize(); }
...
inline procedure* procedure::New(const type *t, uint arity) 
		{ procedure *r = hash.Find(t->GetSerialNo()); return r ? r : new procedure(t, arity); }
inline procedure* procedure::New(const type *t, uint arity, procthunk *thunk) 
	{ procedure *r = hash.Find(t->GetSerialNo()); if( r ) { r->SetThunk(thunk); return r; } else return new proc_builtin(t, arity, thunk); }

/* void CompileProcedures(); */

template <class T>
struct codepair { code *c; T key; T GetKey() { return key; } };

template <class T>
struct bitmappair { Bitmap *bm; T key; T GetKey() { return key; } };
