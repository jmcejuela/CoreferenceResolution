...
class feature
{
	...
	static IntegerHash<feature> hash;			// Hash table for name -> feature
	static VarArray<const feature *> serialtable;
											// Table for serial -> feature

public:
		feature(const char *repname, module *mod, int ip=0, bool exported = true);
  ...
  /// <var>ip</var>：initial priority<br>
	...
	tserial GetKey() const;	// Used in class HashTable
    /// HashTable用のメソッドです。キーとしてnameを返します。<ref>hash.h</ref>
	const char *GetSimpleName() const { return name; }
  /// 名前を取得します。この名前は正式なものではありません。正式な名前を取得するときはGetProperName()を使用してください。
	const char *GetProperName() const { return propername; }
  /// 正式な名前を取得します。
	const char *GetPrintName() const;
  /// 表示用の名前を取得します。
	const char *GetName() const  { return GetProperName(); }
  /// 正式な名前を取得します。
  ...
  /// featureオブジェクトを削除します。
	friend int compfeature(const feature *a, const feature *b) 
		{ int x = a->priority - b->priority; return x ? x : strcmp(a->name, b->name); }
  ...
  /// ２つのfeatureオブジェクトのベースタイプ中での順序を比較します。基本的にはpriorityの順で、priorityが同じなら辞書順になります
	...
	static const feature* Search(tserial t) { return hash.Find(t); }
...
//			{ const feature *r = hash.Find(n); return r ? r : new feature(n, app, ip); }
	...
	static fserial GetSerialCount() { return (fserial)serialtable.GetSize(); }
...
public:
  ...
  /// HashTable用のメソッドです。キーとしてnameを返します。<ref>hash.h</ref>
	const char *GetSimpleName() const { return name; }
  /// nameを返します。
	lstring GetSimpleProperName() const;
  /// ???
	const char *GetProperName() const { return propername; }
  /// propernameを取得します。
	const char *GetPrintName() const;
  ...
  /// feature<var>f</var>に対応するapptypeeのシリアル番号を取得します。
...
// Static member functions
  ...
  /// シリアル番号から対応するtypeオブジェクトを取得します。
	static tserial GetSerialCount() { return (tserial)serialtable.GetSize(); }
...
struct coercode
{
  ...
  /// HashTable用のメソッドです。キーとしてkeyを返します。<ref>hash.h</ref>
};
/// </body></classdef>
...
/// <see></see>
/// <body>
extern IntegerHash<coercode> coerceHash;
/// </body></vardef>
...
inline const coercode * coerceRef(tserial a, tserial b) 
	{ return coerceHash.Find(t2key(a,b)); }
/// </body></funcdef>
...
inline const coercode & coerceData(tserial a, tserial b) 
	{ coercode *f = coerceHash.Find(t2key(a,b)); return f ? *f : coercode_fail; }
/// </body></funcdef>
...
{ 
	coercode *f = coerceHash.Find(t2key(a,b)); 
	if(!f) 
	{ 
		f = new coercode; 
		f->key=t2key(a,b); 
...
lstring Quote(const char *str, bool AllowCapital = false);
