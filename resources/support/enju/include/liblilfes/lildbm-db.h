    ...
    namespace lildbm {
        ...
        protected:
            ...
            bool sanityCheck(std::vector<FSP>& entry) {
                ...
                for(; it != last ; it++, sit++) {
                    if(sit == slast) return false;
                    if(! (*sit)->SanityCheck(*it) ) return false;
                }
                return true;
            }
            bool sanityCheckKey(std::vector<FSP>& entry) {
                ...
                for(; it != last ; it++, sit++) {
                    if(sit == slast) return false;
                    if(! (*sit)->SanityCheckKey(*it) ) return false;
                }
                return true;
            }
            bool Entry2KeyFSList(std::vector<FSP>& entry, FSP keyfs) {
                ...
                for(; it != last; it++, sit++) {
                    ...
                    switch(st) {
                        ...
                        {
                            if(! keyfs.Coerce(cons)) return false;
                            if(! keyfs.Follow(hd).Unify(arg)) return false;
        ...
        public:
            ...
            bool findEntryValue(machine *mach, typename C::dcore_p addr, std::vector<FSP>& arglist) {
                ...
                for(int i = 0; ait != alast ; sit++, ait++, i++) {
                    ...
                    switch(st) {
                        case LDBM_SCHEMA_VALUE_INTEGER: 
                        {
                            if(! ait->Unify(FSP(mach, C::dc2INT(core.read(entry_base + i))))) return false;
                            break;
                        }
                        case LDBM_SCHEMA_VALUE_STRING:
                        {
                            core.readFS(mach, entry_base + i, mark);
                            if(! ait->Unify(FSP(mach, mark.find(core.DDeref(entry_base+i))->second)) ) return false;
                            break;
                        }
                        case LDBM_SCHEMA_VALUE_FLOAT:
                        {
                            if(! ait->Unify(FSP(mach, (float) C::dc2FLO(core.read(entry_base + i))))) return false;
                            break;
                        }
                        case LDBM_SCHEMA_VALUE_TYPE:
                        {
                            if(! ait->Coerce(type::Serial(C::dc2VAR(core.read(entry_base + i)).v))) return false;
                            break;
                        }
                        case LDBM_SCHEMA_VALUE_FS:
                        {
            ...
            bool findall(machine* mach, std::vector<std::vector<FSP> >& r) {
                ...
                while(dcp != base) {
                    ...
                    if(! findEntry(mach, dcp, entry) ) return false;
        ...
        public:
            ...
            bool find(machine* mach, std::vector<FSP>& arglist) {
                if(! sanityCheckKey(arglist) ) return false;
            ...
            bool find(machine* mach, LIT<ldb> lit, std::vector<FSP>& arglist) {
                ...
                if(lit.entry_id != mem::dc2INT(core.read(dcp))) return false;
                return findEntry(mach, dcp, arglist);
            };
            bool insert(machine* mach, std::vector<FSP>& arglist) {
                if(! sanityCheck(arglist) ) return false;
...
// load system information
                ...
                while(dcp != base) {
                    ...
                    if(! findEntry(mach, dcp, arglist) ) return false;
            ...
            static bool load(machine* mach, const char *filename, std::vector<FSP>& data) {
                ...
                if(! db.findall(mach, r)) return false;
        ...
        public:
            ...
            bool find(machine* mach, std::vector<FSP>& arglist) {
                ...
                FSP keyfs(mach);
                if(! Entry2KeyFSList(arglist, keyfs) ) return false;
                ...
                if( assoc_dbp->get(NULL, &key, &val, 0) != 0) return false;
            ...
            bool find(machine* mach, LIT<ldb> lit, std::vector<FSP>& arglist) {
                ...
                if( entry_addr_dbp->get(NULL, &key0, &val0, 0) != 0 ) return false;
                if(lit.entry_id != bdb::dc2INT(core.read(dcp))) return false;
                return findEntry(mach, dcp, arglist);
            };
            bool insert(machine* mach, std::vector<FSP>& arglist) {
            ...
            bool erase(machine* mach, std::vector<FSP>& arglist) {
                ...
                if( assoc_dbp->del(NULL, &key, 0) != 0 ) return false;
                if( entry_addr_dbp->del(NULL, &key2, 0) != 0 ) return false;
            ...
            bool clear() {
                ...
                if( assoc_dbp->truncate(NULL, &assoc_count_p, 0) != 0 ) return false;
                if( entry_addr_dbp->truncate(NULL, &entry_addr_count_p, 0) != 0 ) return false;
                ...
                if( db_class_dbp->put(NULL, &key, &val, 0) != 0 ) return false;

                    // update entry_addr
                Dbt key2((void*) &serial_count, sizeof(serial<bdb>));
                Dbt val2((void*) &base, sizeof(bdb::dcore_p));
                if( entry_addr_dbp->put(NULL, &key2, &val2, 0) != 0 ) return false;

                    // update serial_count
                serial_count.v++;
                Dbt key3((void*)"serial_count", strlen("serial_count") + 1);
                Dbt val3(&serial_count, sizeof(serial<bdb>));
                if( db_class_dbp->put(NULL, &key3, &val3, 0) != 0 ) return false;
 ...
 *
 * $Log: lildbm-db.h,v $
 * Revision 1.28  2011/05/02 08:48:59  matuzaki
 *
 * windows
 *
