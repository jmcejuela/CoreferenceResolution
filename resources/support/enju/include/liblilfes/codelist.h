...
public:
	LabelDef(const char *in, int iv=0)
...
{
public:
	LabelDefCode(const char *in, int iv=0) : LabelDef(in, iv) { }

	virtual void InsertAt(int place, int len) 
			{ if( value >= place ) value += len; }
	virtual void DebugOutputTop() {  }
...
class LabelDefDebug : public LabelDefCode
{
	...
	int line;
public:
	LabelDefDebug(const char *f, int l) : LabelDefCode(NULL) { filename = f; line = l; }
...
class LabelDefDebugStr : public LabelDefCode
{
	lstring str;
public:
	LabelDefDebugStr(const char *f) : LabelDefCode(NULL) { str = f; }
...
public:
	...
	virtual void InsertAt(int place, int len) { if( storeplace > place ) storeplace += len; }
	virtual void Resolve(codelist &) { }
	virtual void Write(codelist &) = 0;
	virtual bool Match(LabelDef *r) { if ( strcmp(r->GetName(), name) == 0 ) { ld = r; return true; } else return false; }
...
// Labelling function.
	...
	void SetLabelDefCode(LabelDefCode *ld) 
			{ ld->SetValue(Current()); SetLabelDef(ld);}
	void SetLabel(const char *name) { SetLabelDefCode(new LabelDefCode(name)); }
