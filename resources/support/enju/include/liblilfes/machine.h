...
public:
		core(const char *iname = NULL, uint is = initCoreSize);
  /// <var>iname</var>：そのcoreのinitial name<br>
  /// <var>is</var>：cell配列のinitial size
		~core();

	void SetName(const char *iname) { name = iname; }
  /// 名前を設定します。
...
//	cell * operator*() CONSTF { return mem; }   // operator* can't assert bound on index, so don't use
	...
	cell & operator[](core_p index) CALCF
			{ ASSERT(index < size); return mem[index]; }
...
//	cell *storep; core_p SHP;   // temporary
	typedef Pair<core_p, code *> ConstraintPair;
	Queue< ConstraintPair > ConstraintQueue;
	...
	void WriteLocal(core_p rp, cell c) 
          {
            ...
            else WriteStack(int2cp(cp2int(FP) - cp2int(rp))+(SF_LOCAL_OFFSET-1), c);
          }
