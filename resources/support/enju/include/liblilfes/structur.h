...
public:
	...
	///<var>im</var> 操作対象の抽象機械
	///<var>d</var> 追加する実数型データ
	FSP(machine *im, const char *s) { mach = im;  addr = mach->GetHP(); mach->CheckHeapLimit(addr); mach->PutHeap(STG2c(mach,s)); }
	...
	char * ReadString()  const { return c2STG(ReadCell()); }
...
protected:
	machine *mach;
	///操作対象の抽象機械です。
	VarArray<AddrRefPair> mark;
	///可変長のarray of HeapPoint& Referencial times of the address
	VarArray<AddrRefPair> share;
	///array of Structure shared address & referential times of the shared???

public:
	ShareList() { mach = NULL; }

	const VarArray<AddrRefPair> &MarkArray() { return mark; }
	///markを返します。
	const VarArray<AddrRefPair> &ShareArray() { return share; }
	///shareを返します。
	...
	void ClearShareCounts() { for( uint i=0; i<share.GetSize(); i++ ) { share[i].ref = 0; } }
	/// shareを全て０にします。
