...
///
        ...
        public:
            ...
            propername(const char* x, const char* y) {module_name = x; simple_name = y;};
            propername(const char* x) {
                module_name = x;
                for(; *x != (char) NULL ; x++) {}
                x++;
                simple_name = x;
            ...
            void Serialize(std::vector<char>& buf) {
                int x = strlen(module_name) + 1;
                int y = strlen(simple_name) + 1;
                for(int i = 0; i < x ; i++)
        ...
        struct hash_propername {
            size_t operator()(propername tn) const { return size_t(__stl_hash_string(tn.module_name) +
                                                                   __stl_hash_string(tn.simple_name)); }
        };

        struct equal_to_propername {
            bool operator()(propername tn1, propername tn2) const {
                return ((strcmp(tn1.module_name, tn2.module_name) == 0) &&
                        (strcmp(tn1.simple_name, tn2.simple_name) == 0));
            }
        };
#ifdef WITH_BDBM
...
/// このクラスは名前とシリアルナンバーのマッピングを保持する
        ...
        public:
            ...
            std::pair<bool, propername> GetPropername(serial<dtype<mem> > ds) {
                ...
                if(t)
                    return std::pair<bool, propername>(true, propername((char*) t->GetModule()->GetName(),
                                                                   (char*) t->GetSimpleName()));
                else
 ...
 *
 * $Log: lildbm-type.h,v $
 * Revision 1.4  2011/05/02 08:48:59  matuzaki
 *
 * windows
 *
