...
/// </p>
/// </desc>
/// <remark>HashTable,VarArrayを使用しています。</remark>
/// <see></see>
/// <body>
class strsymbol
{
	...
	static VarArray<const strsymbol *> serialtable;
	static HashTable<strsymbol> hash;

public:
	strsymbol(machine *mach, const char *name);
	~strsymbol();
		
	const char *GetKey() const { return name; } // for hashtable...
  /// HashTable用のメソッドです。キーとしてnameを返します。
  ...
  /// シリアル番号から対応する strsymbol クラスのポインタを引きます。
	static strsymbol* New(machine *mach, const char *n);
  /// 指定された文字列に対して、すでに対応する strsymbol クラスがあればそのポインタを、なければ新しい strsymbol クラスを確保して返します。
	static int GetSerialCount() { return serialtable.GetSize(); }
...
inline strsymbol* strsymbol::New(machine *mach, const char *n) 
		{ strsymbol *r = hash.Find(n); return r ? r : new strsymbol(mach, n); }
...
/// <see>c2STG</see>
/// <body>
inline cell STG2c(machine *m, const char *s)     { P5("STG2c");   return int2cell( (cell_int)(strsymbol::New(m,s)->GetSerialNo()) | ((cell_int)T_STG << TAG_SHIFT) ); }
...
/// <see>STG2c</see>
/// <body>
inline char * c2STG(cell c)          { P5("c2STG"); ASSERT(IsSTG(c)); return strsymbol::Serial(cell2int(c) & NTAG_MASK)->GetString(); }
/// </body></funcdef>
