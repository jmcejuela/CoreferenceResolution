...
class profbase	// exporting to HashTable<profbase>
{
	unsigned long passcnt; /// 通過回数
	const char *name; /// プロファイル区間名
...
public:
	profbase(const char *n) : passcnt(1), name(n), alltime(0), exctime(0) 
  ...
  /// 再入カウンタを1減らします。
	...
	unsigned long GetPassCount() { return passcnt; }
			/// 通過回数を取得します。
	timebase GetAllTime() { return alltime; }
			/// 総時間を取得します。
	timebase GetExcTime() { return exctime; }
			/// 除外時間を取得します。

	std::ostream& output(std::ostream& os);
			/// このユニットについてのプロファイル結果を出力します。

	static int COMPAREFUNC pbcomp(const void *a, const void *b);
  /// ユニット名の文字列を比較します
};
...
class profuser
{
  ...
  ///開始時間
	...
	profuser *search_internal(const char *name);

public:
	profuser(const char *n);
	~profuser();

	const char *GetName() { return my_base->GetName(); }
