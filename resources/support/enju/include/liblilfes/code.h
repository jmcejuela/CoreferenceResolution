...
class SharedPtr
{
	...
	static IntegerHash<SharedPtr> hash;

	static void term();
	static TermFunction TF;

public:
	SharedPtr(code_ptr icp, bool cs = true) { cp = icp; hash.Add(this); ShouldDelete = cs; }
  /// <var>icp</var>：codeへのポインタ<BR>
  /// <var>cs</var>：上の説明の②かどうか<BR>
	~SharedPtr() { if( ShouldDelete) delete[] cp; }
	ulong GetKey() { return (ulong)cp; }
  /// キーとして対応するcodeへのポインタを返します。

	static IntegerHash<SharedPtr> & Hash() { return hash; }
...
/// <remark></remark>
/// <body>
inline void DeleteCode(code *c) 
{ 
	if( c != NULL && SharedPtr::Hash().Find((ulong)c) == NULL ) 
	{
