...
protected:
  ...
  /// 暗黙のうちにimportされるモジュールのリストです。
	static StringHash<module> modulelist_hash;
...
public:
	explicit module(const char *iname, new_module_type inmt = NMT_NONE, bool Extendable = false);
  /// <var>iname</var>：モジュール名<br>
  /// <var>inmt</var>：<br>
  /// <var>Extendable</var>：extendable(そのモジュールで定義される全ての型について他のモジュールで subtype の定義を許す)かどうか<br>
	~module() { }

	bool IsRegistered() { return *name; }
  /// そのモジュールがモジュールリストに登録されているかどうかを調べます。
	void Register(const char *str = NULL);
  /// モジュールをモジュールリストに追加します。この時、そのモジュールの名前が決まります。
	const char *GetKey() const { return name; }
  /// ハッシュ用のメソッドです。キーとして名前を返します。
	const char *GetName() const { return name; }
  /// 名前を取得します。
	const char *GetDefName() const { return defname; }
  /// モジュール定義がない場合のデフォルト名を返します。
	void Add(type *t) { if( ! IsRegistered() ) Register(); hash.AddOrReplace(t); }
  /// 型<var>t</var>をそのモジュールに追加します。
	void Delete(type *t);
  /// 型<var>t</var>をそのモジュールから削除します。
	type *Search(const char *str) const { ASSERT(this != NULL); return hash.Find(str); }
  /// 型の名前が<var>str</var>である型を取得します。
        const feature *SearchFeature(const char *str) const;
  ...
  /// 再帰的に読み込まれたときの、読み込み元（親）のモジュールを取得します。
	...
	static void SetModulePrefix(lstring prefixname) { module_prefix = prefixname; }
  /// lstring<var>prefixname</var>をmodule_prefixにします。
	static const lstring & GetModulePrefix(void) {return module_prefix;} 
  /// module_prefixを取得します。
  ...
  /// モジュール<var>mod</var>を読み込み中のモジュールに設定します。
	static module *SearchModule(const char *name) { module *m = modulelist_hash.Find(module_prefix + name); return m ? m : modulelist_hash.Find(name); }
  /// 名前から対応するモジュールを取得します。
  ...
  /// そのモジュールが宣言されているかどうかを調べて、宣言されていなければエラーを出して、defnameで登録します。
	static module* new_module(machine &m, const char *biname, lstring modname, new_module_type nmt, module *prev = NULL, module** importlist = NULL);
