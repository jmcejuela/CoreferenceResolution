...
public:
			pcutregion(const char *ilabel, ptree *g) : ptree(), label(ilabel), goals(g), tempid(-1) { }
...
class pinfo_var
{
	...
	///
public:
		pinfo_var(const char *in, int use, VarState st = PV_FIRST ) : name(in) { state = st; firstuse = lastuse = use; usetime = 1; }
	const char *GetKey() { return (const char *)name; } // for hash...
...
struct parseinfo
{
	...
	HashTable<pinfo_var> hash;
	///
	HashTable<pinfo_var> hashcut;
	///
	int ntemp;
	///
	int maxntemp;
